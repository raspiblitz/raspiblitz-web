...
describe("util", () => {
  ...
  test("saveSettings should save settings", () => {
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith(SETTINGS_KEY, JSON.stringify({ lang: "de" }));
  });

  test("saveSettings should save metge with existing settings", () => {
    vi.mocked(localStorage.getItem).mockReturnValue(JSON.stringify({ lang: "en" }));
