      ...
      <div className='bd-card transition-colors'>
        ...
        <div className='flex flex-col overflow-hidden py-4'>
          ...
          <div className='flex'>
            <a
              className={`${
                showTorAddr ? 'w-10/12 overflow-hidden overflow-ellipsis text-blue-400 underline' : 'w-10/12 text-blur'
              }`}
              href={`//${torAddress}`}
              target='_blank'
              rel='noreferrer'
            ...
            <Tooltip
              overlay={<div>{clippedTor ? t('wallet.copied') : t('wallet.copy_clipboard')}</div>}
              placement='top'
            >
              <ClipboardIcon className='inline-flex justify-self-end h-6 w-2/12 cursor-pointer' onClick={copyTor} />
            </Tooltip>
          </div>
        </div>
        <div className='flex flex-col overflow-hidden py-4'>
          ...
          <div className='flex'>
            <p className={showSshAddr ? 'w-10/12' : 'w-10/12 text-blur'}>{showSshAddr ? sshAddress : HIDDEN_TEXT}</p>
            <Tooltip
              overlay={<div>{clippedSsh ? t('wallet.copied') : t('wallet.copy_clipboard')}</div>}
              placement='top'
            >
              <ClipboardIcon className='inline-flex justify-self-end h-6 w-2/12 cursor-pointer' onClick={copySsh} />
            </Tooltip>
          </div>
        </div>
      </div>
