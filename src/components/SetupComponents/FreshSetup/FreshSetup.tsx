...
const FreshSetup: FC = (props) => {
  ...
  return (
    <SetupContainer>
      <div>
        <input type='radio' id='FRESHSETUP' name='setup' value='FRESHSETUP' className='m-4' />
        <label htmlFor='FRESHSETUP'>{t('setup.fresh.setup')}</label>
        <br />
        <input type='radio' id='FROMBACKUP' name='setup' value='FROMBACKUP' className='m-4' />
        <label htmlFor='FROMBACKUP'>{t('setup.fresh.from_backup')}</label>
        <br />
        <input type='radio' id='SHUTDOWN' name='setup' value='SHUTDOWN' className='m-4' />
